...
@Module.Info(name = "AntiFaggot", category = Module.Category.PLAYER)
public class AntiFaggot extends Module
{
    ...
    public void onUpdate() {
        final BlockPos pos = this.getNearestBed();
        if (pos != null) {
            if (!this.isMining) {
        ...
        for (Double x = maxDist; x >= -maxDist; --x) {
            for (Double y = maxDist; y >= -maxDist; --y) {
                for (Double z = maxDist; z >= -maxDist; --z) {
                    ...
                    if (dist <= maxDist && Wrapper.getWorld().getBlockState(pos).getBlock() == Blocks.STANDING_BANNER && this.canBreak(pos) && pos.getY() >= Wrapper.getPlayer().posY) {
                        maxDist = dist;
                        ret = pos;
                    }
                    else if (dist <= maxDist && Wrapper.getWorld().getBlockState(pos).getBlock() == Blocks.STANDING_SIGN && this.canBreak(pos) && pos.getY() >= Wrapper.getPlayer().posY) {
                        maxDist = dist;
                        ret = pos;
                    }
                    else if (dist <= maxDist && Wrapper.getWorld().getBlockState(pos).getBlock() == Blocks.WALL_SIGN && this.canBreak(pos) && pos.getY() >= Wrapper.getPlayer().posY) {
                        maxDist = dist;
                        ret = pos;
                    }
                    else if (dist <= maxDist && Wrapper.getWorld().getBlockState(pos).getBlock() == Blocks.WALL_BANNER && this.canBreak(pos) && pos.getY() >= Wrapper.getPlayer().posY) {
                        maxDist = dist;
                        ret = pos;
                    }
                }
