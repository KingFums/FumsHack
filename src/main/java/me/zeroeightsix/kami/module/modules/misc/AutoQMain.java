...
public class AutoQMain extends Module {
    ...
    public void onUpdate() {
        ...
        if (mc.player.dimension != 1 && endDi.getValue()) {
            Command.sendWarningMessage("[AutoQMain] &l&6Warning: &r&6You are not in the end. Not running &c/queue main&5.");
//            Command.sendWarningMessage("[AutoQMain] " + mc.player.dimension);
            return;
        }
        if (debug.getValue()) {
            Command.sendChatMessage("&5Run &c/queue main&5 at " + System.currentTimeMillis());
        }
